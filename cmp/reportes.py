...
def imprimir_compra(request, compra_id):
    ...
    # Create a Django response object, and specify content_type as pdf
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = 'inline; filename="report.pdf"' # Muestra el archivo antes de imprimir
    # response['Content-Disposition'] = 'attachment; filename="report.pdf"' --->>>> imprime el archivo directamente
